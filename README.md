...
注意：
- API 啟動後會在背景載入模型與索引。此期間 `/` 會顯示 `"status": "loading"`，建議前端 UI 加入等待提示。
...
|----------|-------|--------------------------|
| `/`      | GET   | API 服務狀態與資訊          |
| `/query` | GET   | 查詢 Blender 手冊         |

### 狀態說明

API 啟動後 `/` 端點會回傳當前狀態，`status` 會有以下幾種可能：
...
| `loading`  | 模型與向量資料載入中                 |
| `ok`       | 模型與索引載入完成，已可查詢         |
| `error`    | 啟動過程中發生錯誤，請查看後端日誌     |

此機制可搭配前端 UI 做出 Loading 狀態提示或重試。

### `/`

以下是使用 TypeScript 訪問根端點的示例代碼，返回 API 的API服務狀態與資訊：
...
getApiInfo()
  ...
  .catch(error => console.error('獲取 API 訊息時出錯:', error));
```
